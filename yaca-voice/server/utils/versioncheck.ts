...
export const checkVersion = async () => {
  ...
  for (let i = 0; i < parsedVersion.length; i++) {
    const current = parseInt(parsedVersion[i]);
    const latest = parseInt(parsedLatestVersion[i]);

    if (current !== latest) {
      if (current < latest) {
        console.error(`[YaCA] You are running an outdated version of YaCA. (current: ${currentVersion}, latest: ${latestVersion}) \r\n ${data.html_url}`);
      } else {
        break;
      }
    }
